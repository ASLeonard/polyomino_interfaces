...
def plotParamGrid(Ts=[0,5,15,30,50],Ys=[0,2,5,10]):
     f,axarr=plt.subplots(len(Ys),len(Ts),sharex=True,sharey=True)
     ...
     for (gamma,T),ax in zip(product(Ys,Ts),axarr.flatten()):
          ...
          if (16,0) not in d.evo_strs:
               continue
          for (bond,new_bond),strs in d.evo_strs[(16,0)].items():
...
def plotPhaseSpace(evo_data,low=-2,high=2,res=250):
     ...
     for ctp in filter(None,sx):
          ...
          for (pure,phen,phen_source),phase_p in ctp.items():
               ...
               if phen==(12,0):                    
                    ec='w' #if pure else 'k'
                    ax_2d.scatter(*np.log10(phase_p),marker=phen_mark[phen_source],c=fc,edgecolors=ec,s=m_size,lw=2,hatch=hatch,zorder=ZZ)
                    ...
                    ax_1d.scatter(np.log10(phase_p),FourOne(phase_p),marker=phen_mark[phen_source],c=fc,edgecolors=col,s=m_size*2,zorder=ZZ,hatch=pattern)
